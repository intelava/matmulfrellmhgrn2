...
def fused_recurrent_hgrn_fwd_kernel(
    ...
    p_g = g + i_bh * T * D + o_d
    p_o = o + i_bh * T * D + o_d
    p_new_gate = new_gate + i_bh * T * D + o_d
    ...
    for _ in range(0, T):
        ...
        b_new_gate = tl.load(p_new_gate, mask=mask, other=0).to(tl.float32)
...
class FusedRecurrentHGRNFunction(torch.autograd.Function):
    ...
    @staticmethod
    @contiguous
    def forward(ctx, x, g, new_gate, initial_state=None, output_final_state=False):
        B, H, T, D = x.shape
        ...
        fused_recurrent_hgrn_fwd_kernel[grid](
            x, g, o, new_gate, initial_state, final_state,
            T, D,
...
def fused_recurrent_hgrn(
    ...
    o, final_state = FusedRecurrentHGRNFunction.apply(x, g, new_gate, initial_state, output_final_state)
